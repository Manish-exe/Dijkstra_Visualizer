...
const addEdges = ()=>{
    ...
    //Initializing array for adj matrix rep.
...
//Temp array to store clicked elements to make an edge between the nodes (max size = 2)
let arr = [];
...
const drawLine = (x1,y1,x2,y2,ar) => {
    ...
    console.log(`ar: ${ar}`); //ar is a copy of arr
    //length of line
    ...
    line.id = Number(ar[0]) < Number(ar[1]) 
        ? `line-${ar[0]}-${ar[1]}` 
        : `line-${ar[1]}-${ar[0]}`;
...
const findShortestPath = (el) => {
    ...
    while(unvisited.length){
        ...
        // Relaxation of unvisted edges
        for(j of unvisited){
            if(j === mini) continue;
            if(cost[j] > cost[mini] + dist[mini][j]){
...
const colorEdge = async (el) => {
    if(el.style.backgroundColor !== "#433e3e"){
        await wait(1000);
        el.style.backgroundColor = "#433e3e";
        el.style.height = "5px";
    }
};
